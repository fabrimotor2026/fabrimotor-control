...
function normalizePlannedShipmentStatus(truck) {
...
function draftFromTruck(truck) {
  return {
    ...
    shipmentStatus: normalizePlannedShipmentStatus(truck),
    notes: truck.notes || "",
    status: truck.status,
  };
}
...
export default function ExpeditionPlanner({
  ...
  const [expandedAuditTruckId, setExpandedAuditTruckId] =
    useState("");
        ...
        truckSchedule.map(async (truck) => {
          try {
            const documents = await fetchTruckDocuments(supabase, {
              reference: JOINT_DOCUMENT_REFERENCE,
      ...
      for (const file of files) {
        ...
        await recordTruckAuditEventSafely(supabase, {
          ...
          metadata: {
            file_name: file.name,
            file_size: file.size,
            document_type: documentDraft.documentType,
  ...
  const orderedSchedule = useMemo(() => {
    return [...truckSchedule].sort((first, second) => {
  ...
  const trackingEntries = useMemo(() => {
    return orderedSchedule.map((truck) => {
      const draft =
        draftRows[truck.id] || draftFromTruck(truck);
      const documents = documentsByTruck[truck.id] || [];
      const trackingTruck = {
        ...truck,
        planned_expedition_date:
          draft.plannedExpeditionDate ||
          truck.planned_expedition_date,
        customer_name:
          draft.customerName || truck.customer_name,
      };
  ...
  }, [documentsByTruck, draftRows, orderedSchedule]);

  useEffect(() => {
    if (onTrackingChange) {
      onTrackingChange(trackingEntries);
    }
  }, [onTrackingChange, trackingEntries]);
  ...
  return (
    <FmCard>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <FmSectionTitle
          eyebrow="Logística · V2.38"
          title="Planificador de Expediciones"
          description="Día de expedición, matrículas, brida y documentación de los próximos camiones conjuntos."
        />
      ...
      <p className="mb-4 text-xs font-semibold text-slate-500">
        Cada planificación representa un camión con 49 cajas F1012 y 49 cajas F1013. No se registran costes ni llegada a destino.
      </p>
      ...
      {isCreating && (
        ...
        >
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            ...
            <label className="block">
              <FieldLabel>Día real de expedición</FieldLabel>
              <FmInput
                type="date"
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-4 py-3">
                  ...
                  <div className="flex gap-2">
                    <FmButton
                      ...
                      onClick={() =>
                        setExpandedAuditTruckId((previous) =>
                          previous === truck.id ? "" : truck.id
                        )
                      }
                    >
                      {expandedAuditTruckId === truck.id
                        ? "Ocultar historial"
                        : "Ver historial"}
                    </FmButton>
